...
public class Terminal {
	...
	public void iniciaOperacao() {
		...
		while (opcao != 4) {
			...
			case 6:
				...
				break;
			case 7:  int test = this.meuCaixa.transfere(getInt("Numero da Conta 1"), getInt("Numero da Conta 2"), (double) getInt("Valor"),
					getInt("Senha"));
					if (test == 1) {
	...
	private int getOpcao() {
		...
		do {
			if (this.modoAtual == 1) {
				opcao = getInt(
						"Opcao: 1 - Consulta Saldo, 2 - Saque, 5 - Deposito (Cheque), 6 - Deposito(Cedulas), 7 - Transferencia, 4 - Sair");
				if (opcao != 1 & opcao != 2 & opcao != 4 & opcao != 5 & opcao != 6 & opcao != 7) {
					opcao = 0;
				}
			} else {
